...
T = TypeVar('T', bound='MethionineCarbo')

class MethionineCarbo(ChemicalLoss, metaclass=ABCMeta):
    """
    Base class for methionine-carboxyl chemical interactions in protein structures.
    
    Attributes:
        _atom_idxs_keys (Tuple[str, ...]): The four atoms defining the tetrahedral geometry:
            - 'S1': Sulfur atom of the methionine (SG)
            - 'C1': CB carbon behind the sulfur in methionine
    ...
    _atom_idxs_keys = ( # DO NOT CHANGE THE ORDER OF THESE KEYS, WILL AFFECT THE KDE CALCULATION & BREAK THE CODE
        'S1',  # AA1 sulfur of the methionine
...
@final
class MetAsp(MethionineCarbo):
    """
    Methionine's carboxyl group forms a bond with an aspartic acid carboxyl group.
    """
    _method = "MetAsp"
    ...
    def get_loss_instances(cls,
        ...
        donor_atom_groups = {
            'S1': ['S'],        # Methionine's sulfur
            'C1': ['C4'],        # Methionine's carbon behind the sulfur
        }
...
@final
class MetGlu(MethionineCarbo):
    """
    Methionine's carboxyl group forms a bond with a glutamic acid carboxyl group.
    """
    _method = "MetGlu"
...
@final
class MetCTerm(MethionineCarbo):
    """
    Methionine's carboxyl group forms a bond with a c-terminal carboxyl group.
    """
    _method = "MetCTerm"
